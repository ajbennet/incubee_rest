...
public class SignupController_V10 {
	...
	private static final Logger logger = LoggerFactory.getLogger(SignupController_V10.class);
		...
		if (token != null && token.getId() != null) {
			...
			if (user == null) {
				...
				if (createdUser) {
					...
					return new ResponseEntity<SignupResponse>(response, HttpStatus.INTERNAL_SERVER_ERROR);
				}
			} else {
				...
				return new ResponseEntity<SignupResponse>(response, HttpStatus.CONFLICT);
			}
		}
		response.setStatusCode(SignupResponse.TOKEN_NOT_FOUND);
		response.setStatusMessage("Token not found");
		return new ResponseEntity<SignupResponse>(response, HttpStatus.BAD_REQUEST);
	}
